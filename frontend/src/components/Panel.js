...
const Panel = ({
  ...
  return (
    <div className="client-section">
        ...
        <div className="button-section">
          <ul className="selection-container-list">
            {deviceList.map((device, index) => (
              <li
                key={index}
                className="selection-container-list-item"
                data-device-type={device.device_type}
              >
                <button
                  className="selection-btn"
                  style={{ backgroundColor: device.color }}
                  onClick={() => handlePanelSelection(device.device_name)}
                >
                  {device.device_name}
                </button>
              </li>
      ...
      {selectedDeviceData &&
        selectedDeviceData.canvus &&
        selectedDeviceData.canvus.length > 0 && (
          <div className="col">
      ...
      {selectedDeviceData &&
        selectedDeviceData.device_table &&
        Object.keys(selectedDeviceData.device_table).length > 0 && (
          <table className="device-table">
            <tbody>
              ...
              {Object.entries(selectedDeviceData.device_table).map(
                ([key, value], index) => (
                  <tr key={index}>
                    <td>{key}</td>
                    <td>{value}</td>
