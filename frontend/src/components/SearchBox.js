...
const Autocomplete = ({ suggestions, onSuggestionSelected, setSelectedClientLocation, clients }) => {
  ...
  return (
    <div className={`autocomplete`}>
      ...
      {filteredSuggestions.length > 0 && (
        <div className="autocomplete-items" style={{ display: inputVisible ? "block" : "none" }}>
...
const SearchBox = ({clients, onClientSelected, setSelectedClientLocation }) => {
  ...
  return (
    <form className="searchBox" autoComplete="off" onSubmit={handleSubmit} >
      <Autocomplete suggestions={clientNames} onSuggestionSelected={onClientSelected} setSelectedClientLocation={setSelectedClientLocation} clients={clients}/>
    </form>
  );
};
